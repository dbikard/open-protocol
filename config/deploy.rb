...
on :load do
  ...
  set :copy_exclude, [".git/*", ".bundle/*", "log/*", "vendor/app_gems"]
end
...
CUSTOM_APP_PACKAGES = %w[mysql-client libxml2 libxml2-dev libxslt1-dev imagemagick]
after "rubber:bootstrap", "install_custom_packages"
task :install_custom_packages do
  rubber.sudo_script "install_custom_packages", "export DEBIAN_FRONTEND=noninteractive; apt-get -q -o Dpkg::Options::=--force-confold -y --force-yes install #{CUSTOM_APP_PACKAGES.join(" ")}"
end